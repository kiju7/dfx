...
QC 는 *정적 분석만으로 finding 내지 않음*. 코드 read 로 의심 패턴 식별 후 **실제 측정한 결함만** report.
...
1. **Large input 으로 변경된 함수 / 엔드포인트 호출** — 10⁴, 10⁵, 10⁶ size
2. **Concurrent 부하**: `ab -n 1000 -c 50 <url>`, `wrk`, `xargs -P` 로 동시 요청
3. **프로파일러 실행**:
   - Node: `node --prof <script>` → `--prof-process`
   - Python: `py-spy record -- python <script>`
   - Go: `go test -cpuprofile` + `go tool pprof`
...
**명백히 코드만으로 자명** (예: 루프 안 sync I/O 100% 호출) 만 Phase 2 skip — judgment.

## Phase 3: 결과 기반 finding 확정
- **측정값 있는** 결함만 finding (e.g., "p99 latency 1.2s, baseline 50ms")
- 측정 없는 의심 → `nit` 또는 제외
- 측정값 + 명령 + baseline 비교를 `detail_md` 에 포함
