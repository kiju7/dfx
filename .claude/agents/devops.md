...
1. 의도를 포착하는 최소 reproducer 먼저 작성
   - GitHub Actions → `act` 같은 로컬 러너로 미니 워크플로 검증, 또는 분리된 임시 workflow 파일에 핵심 step 만 추출
   - Dockerfile → 최소 base image + 핵심 RUN 만 담은 `Dockerfile.verify` 로 build
   - 셸 스크립트 → `/tmp/forge-verify-<ts>/` 에 격리해 dry-run
2. reproducer 가 변경 전 상태에서 fail 하는지 확인
3. 본 인프라 파일에 변경 적용
4. 본 파일 syntax / lint 통과 (`actionlint`, `hadolint`, `shellcheck` 등 있으면)
